...
def build_digest(trace: dict, tree: dict, path_map: dict, decl_src: dict) -> str:
    ...
    lines.append("")
    lines.append("1. 读论文/README 抓 story_line")
    lines.append("2. 读 `<root>` 的 forward() 源码，按语法顺序定 pipeline.stages 的 Y-shape")
    lines.append("3. 给每个顶层 node 起 concept_name + purpose_line (Step 3-4)")
    lines.append("4. 标心脏方块 (is_heart) + 训练 only (training_only) (Step 7)")
    lines.append("5. 给关键 leaf 补 formula / callout / what (Step 5)")
    lines.append("6. 把改好的 tree_skeleton.py 重命名/移到 `render/tree_<repo>.py`，再 import 进 render_html.py")
    return "\n".join(lines)
